    ...
    <div class="global__brand">
      <div class="global__brand1">
        <div class="choose__brand__block">
          <div class="choose__brand">Выберите Модель</div>
          <div class="choose__brand">2/3</div>
        </div>
              ...
              <p class="news-item" id="<?= $this->GetEditAreaId($arItem['ID']); ?>">
                ...
                <? if ($arParams["DISPLAY_NAME"] != "N" && $arItem["NAME"]) : ?>
                  <? if (!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])) : ?>
                    ...
                    <label for="model<?= $arItem["ID"] ?>"><a href="#gosnumber" class="user__popup__lvl__2__link pop-mod" onclick='goNextPop(); goUploadAjax(0, "0", "<?= $arItem["NAME"] ?>", "0");'><?= $arItem["NAME"] ?></a></label>
                  <? else : ?>
      ...
      <div class="global__brand2">
        <div class="btn__block">
          <a href="#brand" class="button user__popup__lvl__2__link" onclick="goBackPop(brand)">НАЗАД</a>
        </div>
      </div>
    </div>
  </div>
