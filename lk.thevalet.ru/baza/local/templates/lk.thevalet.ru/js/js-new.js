            ...
            if(FILE.type.indexOf('image/') == 0)
              {const READER = new FileReader();
               READER.addEventListener('load', function(event)
                 {const REQUEST = new XMLHttpRequest(), FILE_TO = ELEMENT.getAttribute('file-to');
                  let extraGetParametersPostfix = '';
                  switch(FILE_TO)
                    {case 'vehicle-photo':
                       extraGetParametersPostfix += '&' + ['id', window.insertedVehicleId].join('=');
                       break;                    
                    }
                  REQUEST.open('POST', '/server.php?action=file-to&target=' + FILE_TO + extraGetParametersPostfix);
